import { iikoExternalServerApi } from 'helpers/iiko/iikoExternalServerApi'
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    switch (path[0]) {
      case 'organizations':
        const organizations = await iikoExternalServerApi.getOrganizations()
        // console.log(organizations)
        res.status(200).json(organizations)
        break

      case 'reserve_available_organizations':
        const reserve_available_organizations = await iikoExternalServerApi.getReserveOrganizations()
        // console.log(organizations)
      ...
      case 'terminal-groups':
        ...
        const terminalGroups = await iikoExternalServerApi.getTerminalGroups(organizationIds)
      ...
      case 'reserve_available_terminal_groups':
        ...
        const reserve_available_terminal_groups = await iikoExternalServerApi.getAvailableTerminalGroups(organizationIds)
      ...
      case 'reserve_available_restaurant_sections':
        ...
        const reserve_available_restaurant_sections = await iikoExternalServerApi.getAvailableRestaurantSections(terminalGroupIds)
      ...
      case 'reserve_restaurant_sections_workload':
        ...
        const reserve_restaurant_sections_workload = await iikoExternalServerApi.getRestaurantSectionsWorkload(restaurantSectionIds, dateFrom, dateTo)
      ...
      case 'reserve_status_by_id':
        ...
        const reserve_status_by_id = await iikoExternalServerApi.getReserveStatusById(organizationId, reserveIds, sourceKeys)
      ...
      case 'command_status':
        ...
        const command_status = await iikoExternalServerApi.getCommandStatus(organizationId, correlationId)
      ...
      case 'nomenclature':
        ...
        const nomenclature = await iikoExternalServerApi.getNomenclature(organizationId)
        res.status(200).json(nomenclature)
        break

      case 'menus_v2':
        const menus_v2 = await iikoExternalServerApi.getMenusV2()
      ...
      case 'menu_by_id_v2':
        ...
        const menu_by_id_v2 = await iikoExternalServerApi.getMenuByIdV2({ externalMenuId, organizationIds })
        res.status(200).json(menu_by_id_v2)
        break

      case 'create_reserve':
        const data = req.body
        const create_reserve = await iikoExternalServerApi.createReserve(data)
