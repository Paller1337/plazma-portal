...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    try {
        ...
        const { ticket, status, newStatus } = req.body.data as { ticket: ISupportTicket, status: TOrderStatus, newStatus: TOrderStatus }
        ...
        const response = await axios.put(`${DEFAULTS.STRAPI.url}/api/support-tickets/${ticket.id}`,
            {
                data: {
                    status: newStatus,
